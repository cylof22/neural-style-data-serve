...
def getEnvs():
    ...
    iterationTimes = os.environ["iterations"]
    networkPath = os.environ["network"]
    return (content, style, output, iterationTimes, networkPath)
...
def main():
    ...
    if options.content is None:
        options.content = envs[0]

    if options.styles is None:
        options.styles = [envs[1]]

    if options.output is None:
        options.output = envs[2]
    
    if envs[3] is not None:
        options.iterations = int(envs[3])
    
    if envs[4] is not None:
        options.network = envs[4]
